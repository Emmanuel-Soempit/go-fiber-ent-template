...
func (cc *CustomerCreate) SetWhatsappID(s string) *CustomerCreate {
	cc.mutation.SetWhatsappID(s)
	return cc
}
...
func (cc *CustomerCreate) check() error {
	if _, ok := cc.mutation.WhatsappID(); !ok {
		return &ValidationError{Name: "whatsapp_id", err: errors.New(`ent: missing required field "Customer.whatsapp_id"`)}
	}
	if v, ok := cc.mutation.WhatsappID(); ok {
